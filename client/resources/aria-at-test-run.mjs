export class TestRun {
    ...
    constructor({ hooks, state }) {
        ...
        this.hooks = {
            ...
            focusCommandUnexpectedBehavior: bindDispatch(
                userFocusCommandUnexpectedBehavior
            ),
            openTestPage: bindDispatch(userOpenWindow),
            postResults: () => {},
            setCommandAdditionalAssertion: bindDispatch(
                userChangeCommandAdditionalAssertion
            ),
            setCommandAssertion: bindDispatch(userChangeCommandAssertion),
            setCommandHasUnexpectedBehavior: bindDispatch(
                userChangeCommandHasUnexpectedBehavior
            ),
            setCommandUnexpectedBehavior: bindDispatch(
                userChangeCommandUnexpectedBehavior
            ),
            setCommandUnexpectedBehaviorMore: bindDispatch(
                userChangeCommandUnexpectedBehaviorMore
            ),
...
export function instructionDocument(resultState, hooks) {
    ...
    const assertions = resultState.commands[0].assertions.map(
            ...
            instructions: {
                ...
                instructions: [
                    [
                        ...
                        {
                            href:
                                'https://github.com/w3c/aria-at/wiki/Configuring-Screen-Readers-for-Testing',
                            description:
        ...
        return {
            ...
            atOutput: {
                ...
                focus:
                    ...
                    focusFirstRequired(),
                change: atOutput =>
                    hooks.setCommandOutput({ commandIndex, atOutput })
            },
            assertionsHeader: {
            ...
            assertions: [
                ...
                ...additionalAssertions.map(
                    bind(additionalAssertionResult, commandIndex)
                )
            ],
            unexpectedBehaviors: {
                description: [
                    'Were there additional undesirable behaviors?',
                    {
                        required: true,
                        highlightRequired:
                            resultStateCommand.unexpected.highlightRequired,
                        description: '(required)'
                    }
                ],
                passChoice: {
                    label:
                        'No, there were no additional undesirable behaviors.',
                    checked:
                            ...
                            (behavior, unexpectedIndex) => {
                                return {
                                    ...
                                    enabled:
                                        ...
                                        HasUnexpectedBehaviorMap.HAS_UNEXPECTED,
                                    tabbable:
                                        resultUnexpectedBehavior.tabbedBehavior ===
                                        unexpectedIndex,
                                    checked: behavior.checked,
                                    focus:
                                        typeof resultState.currentUserAction ===
                                            'object' &&
                                        resultState.currentUserAction.action ===
                                            UserObjectActionMap.FOCUS_UNDESIRABLE
                                            ? resultState.currentUserAction
                                                  .commandIndex ===
                                                  commandIndex &&
                                              resultUnexpectedBehavior.tabbedBehavior ===
                                                  unexpectedIndex
                                            : resultState.currentUserAction ===
                                                  UserActionMap.VALIDATE_RESULTS &&
                                              resultUnexpectedBehavior.hasUnexpected ===
                                                  HasUnexpectedBehaviorMap.HAS_UNEXPECTED &&
                                              resultUnexpectedBehavior.behaviors.every(
                                                  ({ checked }) => !checked
                                              ) &&
                                              focusFirstRequired(),
                                    change: checked =>
                                        hooks.setCommandUnexpectedBehavior({
                                            commandIndex,
                                            unexpectedIndex,
                                            checked
                                        }),
                                    keydown: key => {
                                        const increment = keyToFocusIncrement(
                                            key
                                        );
                                        if (increment) {
    ...
    function assertionResult(commandIndex, assertion, assertionIndex) {
        const resultAssertion =
            resultState.commands[commandIndex].assertions[assertionIndex];
            ...
            passChoice: assertionChoice(resultAssertion, CommonResultMap.PASS, {
                ...
                click: () =>
                    hooks.setCommandAssertion({
                        commandIndex,
                        assertionIndex,
                        result: CommonResultMap.PASS
                    })
            }),
            failChoices: [
                assertionChoice(
                    resultAssertion,
                    AssertionResultMap.FAIL_MISSING,
                    {
                        label: [
                            `No Output `,
                            {
                ...
                assertionChoice(
                    resultAssertion,
                    AssertionResultMap.FAIL_INCORRECT,
                    {
                        label: [
                            `Incorrect Output `,
                            {
     ...
     * @param {number} assertionIndex
     */
    function additionalAssertionResult(
        commandIndex,
        assertion,
        assertionIndex
    ) {
        const resultAdditionalAssertion =
            resultState.commands[commandIndex].additionalAssertions[
                assertionIndex
            ];
        return /** @type {InstructionDocumentResultsCommandsAssertion} */ ({
            description: [
                assertion,
                {
                    required: true,
                    highlightRequired:
                        resultAdditionalAssertion.highlightRequired,
                    description: '(required: mark support)'
                }
            ],
            passChoice: assertionChoice(
                resultAdditionalAssertion,
                AdditionalAssertionResultMap.PASS,
                {
                    label: [
                        'Good Support ',
                        { offScreen: true, description: 'for assertion' }
                    ],
                    click: () =>
            ...
            failChoices: [
                assertionChoice(
                    resultAdditionalAssertion,
                    AdditionalAssertionResultMap.FAIL_SUPPORT,
                    {
                        label: [
                            'No Support ',
                            { offScreen: true, description: 'for assertion' }
 ...
 * @returns {(state: TestRunState) => TestRunState}
 */
export function userChangeCommandAssertion({
    commandIndex,
    assertionIndex,
    result
}) {
        ...
        return {
            ...
            commands: state.commands.map((command, commandI) =>
                commandI !== commandIndex
                    ...
                    : {
                          ...command,
                          assertions: command.assertions.map(
                              (assertion, assertionI) =>
                                  assertionI !== assertionIndex
                                      ? assertion
                                      : { ...assertion, result }
                          )
                      }
        ...
        return {
            ...
            commands: state.commands.map((command, commandI) =>
                commandI !== commandIndex
                    ...
                    : {
                          ...command,
                          additionalAssertions: command.additionalAssertions.map(
                              (assertion, assertionI) =>
                                  assertionI !== additionalAssertionIndex
                                      ? assertion
                                      : { ...assertion, result }
                          )
                      }
 ...
 * @returns {(state: TestRunState) => TestRunState}
 */
export function userChangeCommandHasUnexpectedBehavior({
    commandIndex,
    hasUnexpected
}) {
    return function(state) {
 ...
 * @returns {(state: TestRunState) => TestRunState}
 */
export function userChangeCommandUnexpectedBehavior({
    commandIndex,
    unexpectedIndex,
    checked
}) {
 ...
 * @returns {(state: TestRunState) => TestRunState}
 */
export function userChangeCommandUnexpectedBehaviorMore({
    commandIndex,
    unexpectedIndex,
    more
}) {
...
function isSomeFieldRequired(state) {
    return state.commands.some(
        command =>
            ...
            (command.unexpected.hasUnexpected ===
                HasUnexpectedBehaviorMap.HAS_UNEXPECTED &&
                (command.unexpected.behaviors.every(
                    ({ checked }) => !checked
                ) ||
    ...
    return {
        ...
        table: {
            ...
            commands: state.commands.map(command => {
                ...
                if (
                    allAssertions.some(
                        ({ result }) => result === CommonResultMap.PASS
                    )
                ) {
                ...
                if (
                    ...
                    unexpectedBehaviors = command.unexpected.behaviors
                        ...
                        .map(({ description, more }) =>
                            more ? more.value : description
                        );
                }
 ...
 * @returns {(state: TestRunState) => TestRunState}
 */
export function userFocusCommandUnexpectedBehavior({
    commandIndex,
    unexpectedIndex,
    increment
}) {
    return function(state) {
        const unexpectedLength =
            state.commands[commandIndex].unexpected.behaviors.length;
        const incrementValue = increment === 'next' ? 1 : -1;
        const newUnexpectedIndex =
            (unexpectedIndex + incrementValue + unexpectedLength) %
            unexpectedLength;
            ...
            commands: state.commands.map(command => {
                return {
                    ...
                    assertions: command.assertions.map(assertion => ({
                        ...assertion,
                        highlightRequired:
                            assertion.result === CommonResultMap.NOT_SET
                    })),
                    additionalAssertions: command.additionalAssertions.map(
                        assertion => ({
                            ...assertion,
