...
class SupportInput {
    ...
    static fromCollectedTest(collectedTest) {
        return new SupportInput({
            ats: [
                {
                    key: collectedTest.target.at.key,
                    name: collectedTest.target.at.name
                }
            ],
            applies_to: {},
            examples: []
...
class CommandsInput {
    ...
    getCommands(task, atMode) {
        ...
        let commandsData =
            this._value.commands[task][atMode][assistiveTech.key] || [];
        let commands = [];
    ...
    static fromJSONAndConfigKeys(json, { configInput, keysInput }) {
        return new CommandsInput(
            { commands: json, at: configInput.at() },
            keysInput
        );
    }
                    ...
                    [collectedTest.info.task]: {
                        [collectedTest.target.mode]: {
                            [collectedTest.target.at
                                .key]: collectedTest.commands.map(
                                ({ id, extraInstruction }) =>
                                    extraInstruction
                                        ? [id, extraInstruction]
                                        : [id]
    ...
    static fromQueryParamsAndSupport(queryParams, { supportInput }) {
        ...
        for (const [key, value] of queryParams) {
            if (key === 'at') {
                ...
                displaySubmitButton = decodeBooleanParam(
                    value,
                    displaySubmitButton
                );
            } else if (key === 'showSubmitButton') {
                renderResultsAfterSubmit = decodeBooleanParam(
                    value,
                    renderResultsAfterSubmit
                );
            } else if (key === 'resultFormat') {
                if (
                    value !== 'SubmitResultsJSON' &&
        ...
        if (resultJSON && resultFormat !== 'TestResultJSON') {
            errors.push(
                `resultJSON requires resultFormat to be set to 'TestResultJSON'.`
            );
            resultJSON = null;
        }
...
class ScriptsInput {
    ...
    static scriptsFromSource(script) {
        return {
            [script.name]: new Function('testPageDocument', script.source)
        };
    }
    ...
    static async scriptsFromJsonpAsync(script, dataUrl) {
        return await Promise.race([
            ...
            new Promise((_, reject) =>
                setTimeout(
                    () => reject(new Error('Loading scripts timeout error')),
                    10000
                )
            )
        ]);
    ...
    static async fromCollectedTestAsync({ target: { setupScript } }, dataUrl) {
        ...
        try {
            return new ScriptsInput({
                scripts: ScriptsInput.scriptsFromSource(setupScript)
            });
        } catch (error) {
            try {
                return new ScriptsInput({
                    scripts: await ScriptsInput.scriptsFromModuleAsync(
                        setupScript,
                        dataUrl
                    )
                });
                ...
                try {
                    return new ScriptsInput({
                        scripts: await ScriptsInput.scriptsFromJsonpAsync(
                            setupScript,
                            dataUrl
                        )
                    });
...
class UnexpectedInput {
    ...
    static fromBuiltin() {
        return new UnexpectedInput({
            behaviors: [
                ...UNEXPECTED_BEHAVIORS.map(description => ({
                    description,
    ...
    static fromJSONCommandsConfigKeysTitleUnexpected(
        ...
        return new BehaviorInput({
            behavior: {
                ...
                assertions: (json.output_assertions
                    ? json.output_assertions
                    : []
                ).map(assertionTuple => ({
        ...
        return new BehaviorInput({
            behavior: {
                ...
                setupScriptDescription: target.setupScript
                    ...
                    : '',
                setupTestPage: target.setupScript
                    ? target.setupScript.name
                    : undefined,
                commands: commandsInput.getCommands(info.task, target.mode),
                assertions: assertions.map(
                    ({ priority, expectation: assertion }) => ({
                        priority,
                        assertion
                    })
                ),
...
export class TestRunInputOutput {
    ...
    setBehaviorInputFromJSONAndCommandsConfigKeysTitleUnexpected(behaviorJSON) {
        invariant(
            ...
            this.setBehaviorInputFromJSONAndCommandsConfigKeysTitleUnexpected
                .name
        );
        invariant(
            this.configInput !== null,
        ...
        this.setBehaviorInput(
            BehaviorInput.fromJSONCommandsConfigKeysTitleUnexpected(
                behaviorJSON,
                {
    ...
    async setInputsFromCollectedTestAsync(collectedTest, dataUrl) {
        const pageUriInput = PageUriInput.fromPageUri(
            collectedTest.target.referencePage
        );
        ...
        const scriptsInput = await ScriptsInput.fromCollectedTestAsync(
            collectedTest,
            dataUrl
        );
        ...
        const commandsInput = CommandsInput.fromCollectedTestKeys(
            collectedTest,
            { keysInput }
        );
        const behaviorInput = BehaviorInput.fromCollectedTestCommandsKeysUnexpected(
            collectedTest,
            {
                commandsInput,
                keysInput,
    ...
    setKeysInputFromBuiltinAndConfig() {
        ...
        this.setKeysInput(
            KeysInput.fromBuiltinAndConfig({ configInput: this.configInput })
        );
    }
    ...
    testRunState() {
        ...
        let state = {
            ...
            commands: test.commands.map(
                command => /** @type {import("./aria-at-test-run.mjs").TestRunCommand} */ ({
                    ...
                    additionalAssertions: test.additionalAssertions.map(
                        assertion => ({
                            description: assertion.assertion,
                            highlightRequired: false,
                            priority: assertion.priority,
                    ...
                    unexpected: {
                        ...
                        tabbedBehavior: 0,
                        behaviors: test.unexpectedBehaviors.map(
                            ({ description, requireExplanation }) => ({
                                description,
                                checked: false,
                                more: requireExplanation
                                    ? { highlightRequired: false, value: '' }
                                    : null
                            })
                        )
                    }
        ...
        if (this.configInput.resultJSON()) {
            state = this.testRunStateFromTestResultJSON(
                this.configInput.resultJSON(),
                state
            );
        }
    ...
    submitResultsJSON(state) {
            ...
            summary: {
                1: {
                    pass: countAssertions(
                        ({ priority, result }) =>
                            priority === 1 && result === CommonResultMap.PASS
                    ),
                ...
                2: {
                    pass: countAssertions(
                        ({ priority, result }) =>
                            priority === 2 && result === CommonResultMap.PASS
                    ),
                    fail: countAssertions(
                        ({ priority, result }) =>
                            priority === 2 && result !== CommonResultMap.PASS
                ...
                unexpectedCount: countUnexpectedBehaviors(
                    ({ checked }) => checked
                )
            },
        ...
        function countUnexpectedBehaviors(filter) {
            return state.commands.reduce(
                (carry, command) =>
                    carry + command.unexpected.behaviors.filter(filter).length,
                0
            );
        }
                    ...
                    .map(behavior =>
                        behavior.checked
                            ? {
                                  ...
                                  otherUnexpectedBehaviorText: behavior.more
                                      ? behavior.more.value
                                      : null
                              }
                            : null
        ...
        return {
            ...
            commands: state.commands.map((command, commandIndex) => {
                ...
                return {
                    ...
                    atOutput: {
                        highlightRequired: false,
                        value: scenarioResult.output
                    },
                    assertions: command.assertions.map(
                        (assertion, assertionIndex) => {
                            const assertionResult =
                                scenarioResult.assertionResults[assertionIndex];
                            return {
                                ...assertion,
                                highlightRequired: false,
                            ...
                            behavior => {
                                ...
                                return {
                                    ...
                                    more: behavior.more
                                        ? {
                                              highlightRequired: false,
                                              value: behaviorResult
                                                  ? behaviorResult.otherUnexpectedBehaviorText
                                                  : ''
                                          }
                                        : behavior.more
                                };
