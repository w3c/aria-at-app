...
const TestQueue = () => {
  ...
  const [isShowingDeleteResultsModal, setDeleteResultsModal] = useState(false);
  ...
  useEffect(() => {
    if (data) {
      ...
      setTesters(
        users.filter(
          tester =>
            tester.roles.includes('TESTER') || tester.roles.includes('ADMIN')
        )
      );
      setAts(ats);
  ...
  const renderAtBrowserList = (title = '', testPlanReports = []) => {
    ...
    return (
      <div key={title}>
        ...
        <Table className="test-queue" aria-labelledby={tableId} bordered>
          <thead>
            <tr>
            ...
            {testPlanReports.map(testPlanReport => {
              ...
              return (
                <TestQueueRow
                  ...
                  latestTestPlanVersions={latestTestPlanVersions}
                  triggerDeleteTestPlanReportModal={
                    triggerDeleteTestPlanReportModal
                  }
                  triggerDeleteResultsModal={triggerDeleteResultsModal}
  ...
  return (
    <Container id="main" as="main" tabIndex="-1">
      ...
      <h1>Test Queue</h1>
      {emptyTestPlans && (
        <h2 data-testid="test-queue-no-test-plans-h2">{noTestPlansMessage}</h2>
      )}
      {emptyTestPlans && !isAdmin && isSignedIn && (
        ...
        >
          Please configure your preferred Assistive Technologies in the{' '}
          {settingsLink} page.
        </Alert>
      )}
