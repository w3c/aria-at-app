...
const BotRunTestStatusList = ({ testPlanReportId, runnableTestsLength }) => {
  ...
  const [numTestsCompleted, numTestsQueued, numTestsCancelled] = useMemo(() => {
    const res = [0, 0, 0];
    if (
      botTestPlanRuns &&
      ...
      for (let i = 0; i < botTestPlanRuns.length; i++) {
        ...
        switch (status) {
          ...
          case 'QUEUED':
            res[1] +=
              runnableTestsLength - botTestPlanRuns[i].testResults.length;
            break;
          case 'CANCELLED':
            res[2] +=
              runnableTestsLength - botTestPlanRuns[i].testResults.length;
      ...
      if (res[0] + res[2] === runnableTestsLength * botTestPlanRuns.length) {
        stopPolling();
      }
    }
    return res;
