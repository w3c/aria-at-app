...
const TestQueueRow = ({
  ...
  const [removeTestPlanReport] = useMutation(REMOVE_TEST_PLAN_REPORT_MUTATION);
  ...
  const primaryTestPlanRunOptions = draftTestPlanRuns
    .slice()
    .sort((a, b) =>
      a.tester.username.toLowerCase() < b.tester.username.toLowerCase() ? -1 : 1
    )
    .map(run => ({
  ...
  const renderAssignedUserToTestPlan = () => {
    const titleElement = (
      <>
        ...
        {runnableTestsLength === 0 || runnableTestsLength > 1 ? `s` : ''})
      </>
    );
    ...
    if (currentUserAssigned)
      return (
        <>
          <Link className="test-plan" to={`/run/${currentUserTestPlanRun.id}`}>
            {titleElement}
          </Link>
        </>
      );
  ...
  const renderAssignMenu = () => {
    return (
      <AssignTesterDropdown
        onChange={async () => {
          focusButtonRef.current = dropdownAssignTesterButtonRef.current;
          await triggerTestPlanReportUpdate();
        }}
          ...
          {draftTestPlanRuns
            ...
            .map(({ tester }) => {
              return (
                <Dropdown.Item
                  role="menuitem"
                  href={`/run/${getTestPlanRunIdByUserId(tester.id)}?user=${
                    tester.id
                  }`}
                  key={nextId()}
                >
  ...
  const renderDeleteMenu = () => {
    if (testPlanRunsWithResults.length) {
      return (
        <>
          <Dropdown aria-label="Delete results menu" focusFirstItemOnShow>
            <Dropdown.Toggle
                  ...
                  <Dropdown.Item
                    ...
                    onClick={() => {
                      triggerDeleteResultsModal(
                        ...
                        async () => {
                          await triggerLoad(async () => {
                            await handleRemoveTesterResults(id);
            ...
            <ManageBotRunDialogWithButton
              ...
              onDelete={() => {
                triggerDeleteResultsModal(
                  ...
                  async () => {
                    await triggerLoad(async () => {
                      await handleRemoveTesterResults(botTestPlanRun.id);
                    }, 'Removing Test Results');
                    dropdownDeleteTesterResultsButtonRef.current.focus();
                  }
                );
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  const primaryTestPlanRunId =
                    primaryTestPlanRunOptions[0].testPlanRunId;

                  if (primaryTestPlanRunOptions.length > 1) {
                    setPrimaryTestPlanRunId(primaryTestPlanRunId);
  ...
  const evaluateLabelStatus = () => {
    ...
    if (isLoading) {
      ...
      labelStatus = (
        <span className="status-label conflicts">{pluralizedStatus}</span>
      );
    } else {
      labelStatus = <span className="status-label not-started">Draft</span>;
    }
  ...
  return (
    <LoadingStatus message={loadingMessage}>
      <tr className="test-queue-run-row">
            ...
            <div className="testers-wrapper">
              ...
              <div className="assign-actions">
                <Button
                  ...
                  variant="secondary"
                  onClick={async () => {
                    focusButtonRef.current = assignTesterButtonRef.current;
                ...
                {draftTestPlanRuns
                  ...
                  .map(draftTestPlanRun => (
                    <TestQueueCompletionStatusListItem
                      ...
                      id={getRowId(draftTestPlanRun.tester)}
                    />
                  ))}
              </ul>
            ) : (
        ...
        <td>
          <div className="status-wrapper">{evaluateLabelStatus()}</div>
          {isSignedIn && isTester && (
            <div className="secondary-actions">{renderSecondaryActions()}</div>
          )}
        </td>
        <td className="actions">
          <div className="test-cta-wrapper">
            {currentUserAssigned && (
              ...
              >
                ...
                currentUserTestPlanRun.testResultsLength < runnableTestsLength
                  ? 'Continue testing'
                  : 'Start testing'}
              </Button>
            )}
          ...
          {isSignedIn && isTester && (
            <div className="secondary-actions">
              ...
              {(!isAdmin && currentUserTestPlanRun.testResultsLength > 0 && (
                <Button
