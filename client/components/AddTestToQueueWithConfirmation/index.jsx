...
function AddTestToQueueWithConfirmation({
  ...
  const oldTestPlanVersions =
    existingTestPlanReportsData?.oldTestPlanVersions?.filter(({ metadata }) => {
      return (
        metadata.testFormatVersion ===
        existingTestPlanReportsData?.existingTestPlanVersion?.metadata
          .testFormatVersion
      );
    }) || [];
  ...
  const addTestToQueue = async ({ copyResultsFromTestPlanVersionId } = {}) => {
    let tpr;
