...
function AddTestToQueueWithConfirmation({
  ...
  if (
    ...
    latestOldVersion = existingTestPlanReportsData?.oldTestPlanVersions?.reduce(
      ...
      actions.push({
        ...
        onClick: async () => {
          await addTestToQueue(
            canUseOldResults
              ? {
                  copyResultsFromTestPlanReportId: latestOldVersion.id
                }
              : {}
          );
  ...
  const addTestToQueue = async ({ copyResultsFromTestPlanReportId } = {}) => {
    ...
    await triggerLoad(async () => {
      ...
      const testPlanReport =
        res?.data?.findOrCreateTestPlanReport?.populatedData?.testPlanReport ??
        null;
      tpr = testPlanReport;
