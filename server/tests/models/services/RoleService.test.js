...
describe('RoleModel Data Checks', () => {
  ...
  it('should only have 3 named roles (ADMIN, TESTER, and VENDOR)', async () => {
    ...
    expect(roles).toContainEqual(expect.objectContaining({ name: 'VENDOR' }));
  });
  ...
  it('should create, update and add a user to a new role', async () => {
    await dbCleaner(async transaction => {
      ...
      expect(addedUserRoleUsers.length - 1).toBe(removedUserRoleUsers.length);
    });
  });
  ...
  it('should create and remove a role', async () => {
    await dbCleaner(async transaction => {
      ...
      // after role created
      expect(rolesAfterCreatedLength).toEqual(rolesBeforeCreatedLength + 1);
