...
describe('RunService', () => {
    describe('RunService.configureRuns', () => {
        ...
        it('should copy over testers for the same AT/ATVersion, Browser/BrowserVersion, and ApgExample on a new test version', async () => {
            await dbCleaner(async () => {
                const browserVersionNumber = '1.2.3';
                const atVersionNumber = '3.2.1';
                ...
                const testerToRun = await db.TesterToRun.create({
                    run_id: activeRunOlderTestVersion.id,
                    user_id: user.id
                });
                ...
                // Verify that the the user has been copied to the new run
                const runId = parseInt(keys[0]);
                expect(activeRuns[runId]).toEqual({
                    id: runId,
                    browser_id: browser.id,
        ...
        it('should not copy over tester when the run with older version is marked final', async () => {
            await dbCleaner(async () => {
                const browserVersionNumber = '1.2.3';
                const atVersionNumber = '3.2.1';
                ...
                let tech = await db.BrowserVersionToAtVersion.create({
                    ...
                    browser_version_id: browserVersion.id,
                    active: true,
                    run_status_id: runStatusFinal.id
                });
