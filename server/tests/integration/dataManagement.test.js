...
describe('data management', () => {
  ...
  it('updates test plan version and copies results from previous version reports', async () => {
    await dbCleaner(async transaction => {
      ...
      const [oldModalDialogVersion] = testPlanVersions.testPlanVersions.filter(
        e => e.testPlan.directory === 'modal-dialog' && e.phase === 'CANDIDATE'
      );
      ...
      const [newModalDialogVersion] = testPlanVersions.testPlanVersions.filter(
    ...
    await dbCleaner(async transaction => {
      ...
      const {
        findOrCreateTestPlanReport: {
          populatedData: { testPlanVersion: newModalDialogVersionInDraft }
        }
  ...
  it('updates test plan version but removes marked as final for test plan report when new report has incomplete runs', async () => {
    await dbCleaner(async transaction => {
      ...
      const oldCommandButtonVersionMarkedFinalReportsCount =
        oldCommandButtonVersion.testPlanReports.reduce(markedFinalAtReduce, 0);
      ...
      expect(newCommandButtonVersionInDraftMarkedFinalReportsCount).toEqual(
        oldCommandButtonVersionMarkedFinalReportsCount - 1
      );
    });
  });
});
