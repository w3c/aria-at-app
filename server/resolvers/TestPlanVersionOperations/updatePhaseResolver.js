...
const updatePhaseResolver = async (
  ...
  if (phase === 'CANDIDATE') {
    ...
    ats.forEach(at => {
      const browsers =
        phase === 'CANDIDATE' ? at.candidateBrowsers : at.recommendedBrowsers;
      browsers.forEach(browser => {
        if (!reportsByAtAndBrowser[at.id]?.[browser.id]) {
          missingAtBrowserCombinations.push(`${at.name} and ${browser.name}`);
        }
      });
    });
  ...
  if (phase === 'RD')
    ...
    const candidatePhaseReachedAtValue = candidatePhaseReachedAt || new Date();
  ...
  if (oldTestPlanVersion && phase === oldTestPlanVersion.phase) {
    ...
    if (deprecatedAt) deprecatedAt.setSeconds(deprecatedAt.getSeconds() - 120);
