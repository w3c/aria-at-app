...
const updateTestPlanReportTestPlanVersionResolver = async (
    ...
    tests: newTestPlanVersionData.tests.map(
      ({ assertions, atIds, id, scenarios, title }) => {
        return {
          ...
          scenarios: scenariosResolver({ scenarios }, { atId }, context).map(
            ({ commandIds }) => {
              return {
            ...
            return {
              ...
              scenarioResults: scenarioResults.map(
                ({ output, assertionResults, unexpectedBehaviors }) => ({
                  output,
                  assertionResults: assertionResults.map(({ passed }) => ({
                    passed
                  })),
  ...
  const candidatePhaseReachedAt = currentTestPlanReport.candidatePhaseReachedAt;
  ...
  for (const testPlanRun of runsWithResults) {
    ...
    for (const testResult of testPlanRun.testResults) {
      ...
      if (savedData.errors) console.error('savedData.errors', savedData.errors);
    }
  }
