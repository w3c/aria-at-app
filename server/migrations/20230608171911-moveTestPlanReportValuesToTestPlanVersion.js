...
module.exports = {
  async up(queryInterface, Sequelize) {
              ...
              assertionResults: scenarioResultSkeleton.assertionResults.map(
                (assertionResultSkeleton, assertionResultIndex) => {
                  const assertionResult =
                    scenarioResult.assertionResults[assertionResultIndex];
                  return {
    ...
    return queryInterface.sequelize.transaction(async transaction => {
      ...
      const testPlanReportLatestReleasedQuery = async testPlanReportId => {
        ...
        for (const directory in testPlanReportsByDirectory) {
          ...
          for (const report of reports) {
            ...
            if (
              ...
              latestAtBrowserMatchings[uniqueAtBrowserKey] = report;

              if (status === 'CANDIDATE') highestCollectiveStatus = 'CANDIDATE';
            }

            if (
              testPlanVersionId > highestTestPlanVersion ||
              (testPlanVersionId === highestTestPlanVersion &&
                latestAtVersionReleasedAt > latestAtVersionReleasedAtOverall)
            ) {
              highestTestPlanVersion = testPlanVersionId;
              latestAtVersionReleasedAtOverall = latestAtVersionReleasedAt;
              latestCandidateStatusReachedAt = candidateStatusReachedAt;
              latestRecommendedStatusReachedAt = recommendedStatusReachedAt;
              latestRecommendedStatusTargetDate = recommendedStatusTargetDate;
            }
          }
      ...
      const updateTestPlanReportTestPlanVersion = async ({
        ...
        const newTestPlanVersionQuery = await queryInterface.sequelize.query(
          `SELECT
                             *
        ...
        for (let i = 0; i < currentTestPlanReport.testPlanRuns.length; i++) {
          const testPlanRunId = currentTestPlanReport.testPlanRuns[i].id;
                ...
                ({
                  ...
                  return {
                    test: {
                      ...
                      assertions: test.assertions.map(({ priority, text }) => ({
                        priority,
                        text
                      }))
                    },
                    ...
                    scenarioResults: scenarioResults.map(
                      ({ output, assertionResults, unexpectedBehaviors }) => ({
                        output,
      ...
      for (let i = 0; i < Object.values(highestVersions).length; i++) {
        const highestTestPlanVersion = Object.values(highestVersions)[i];
        ...
        const {
          ...
          latestRecommendedStatusTargetDate: recommendedPhaseTargetDate,
          latestAtBrowserMatchings
        } = highestTestPlanVersion;
        ...
        for (const uniqueMatchKey in latestAtBrowserMatchings) {
          const uniqueMatch = latestAtBrowserMatchings[uniqueMatchKey];
          if (uniqueMatch.testPlanVersionId !== highestTestPlanVersionId) {
            // eslint-disable-next-line no-console
            console.info(
            ...
            await updateTestPlanReportTestPlanVersion({
              ...
              testPlanReportAttributes: TEST_PLAN_REPORT_ATTRIBUTES.filter(
