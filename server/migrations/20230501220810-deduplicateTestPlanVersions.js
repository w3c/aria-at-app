...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const regenerateExistingTestResults = async (
      ...
      for (const key in equivalentTestPlanVersionIds) {
        const [keptId, ...unkeptIds] = equivalentTestPlanVersionIds[key];
        ...
        // keptId
        const unkeptTestPlanReports = await queryInterface.sequelize.query(
        ...
        const keptTestPlanVersion = await queryInterface.sequelize.query(
        ...
        const { tests: keptTestPlanVersionTests } = keptTestPlanVersion[0];
                      ...
                      eachScenarioResult.assertionResults.forEach(
                        (eachAssertionResult, assertionIndex) => {
                          eachAssertionResult.assertionId =
                            foundKeptTest.assertions[assertionIndex].id;
    ...
    const removeTestPlanVersionDuplicates = async (
      ...
      await Promise.all(
        equivalentTestPlanVersionIds.map(async ([keptId, ...unkeptIds]) => {
    ...
    const getKnownGitCommits = async () => {
      ...
      for (const testDirectory of testDirectories) {
        ...
        try {
          ...
          console.error('get.commits.error', testDirectory, error.message);
        }
      }
    ...
    const processTestPlanVersionIdsByHashedTests = (
      ...
      for (const directory in knownGitCommits) {
        ...
        const filteredTestPlanVersionsByHashedTestsForDirectory =
          Object.fromEntries(
            Object.entries(testPlanVersionsByHashedTests).filter(
              ...
              ([key, arr]) => arr.some(obj => obj.directory === directory)
            )
          );
        ...
        for (const hash in filteredTestPlanVersionsByHashedTestsForDirectory) {
          filteredTestPlanVersionsByHashedTestsForDirectory[hash].sort(
            (a, b) => {
      ...
      const testPlanVersionIdsByHashedTests =
        getTestPlanVersionIdsByHashedTests(testPlanVersionsByHashedTests);
    ...
    return queryInterface.sequelize.transaction(async transaction => {
      ...
      const uniqueHashCount = Object.keys(testPlanVersionsByHashedTests).length;
      ...
      for (let i = 0; i < iterationsNeeded; i += 1) {
        ...
        await removeTestPlanVersionDuplicates(equivalentIds, transaction);
      }
  ...
  async down(queryInterface /* , Sequelize */) {
    return queryInterface.sequelize.transaction(async transaction => {
      await queryInterface.removeColumn('TestPlanVersion', 'hashedTests', {
        transaction
      });
    });
  }
