  ...
  up: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.transaction(async transaction => {
      ...
      const csvString = testPlanTargetIdData.reduce(
        (prev, current) => `${prev}${current.id},${current.testPlanTargetId}\n`
      );
      fs.writeFile(`${__dirname}/test_plan_target_id.csv`, csvString, err => {
        if (err) {
          console.error(err);
        }
      });

      await queryInterface.removeColumn('TestPlanReport', 'testPlanTargetId', {
        transaction
      });
    });
  },
