  ...
  async up(queryInterface, Sequelize) {
    return queryInterface.sequelize.transaction(async transaction => {
      ...
      for (const testPlanVersion of testPlanVersionsToSetToCandidate) {
        ...
        candidatePhaseReachedAt.setDate(candidatePhaseReachedAt.getDate() + 1);

        const recommendedPhaseTargetDate = new Date(candidatePhaseReachedAt);
      ...
      for (let testPlanVersion of flattenedTestPlanVersions) {
        ...
        if (
          ...
          if (candidatePhaseReachedAt < draftPhaseReachedAt) {
            const newCandidatePhaseReachedAt = new Date(draftPhaseReachedAt);
